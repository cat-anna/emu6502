...
const static std::array<uint8_t, 256> crc8_table = {
    ...
    0xf6, 0xa8, 0x74, 0x2a, 0xc8, 0x96, 0x15, 0x4b, 0xa9, 0xf7, 0xb6, 0xe8, 0x0a, 0x54,
    0xd7, 0x89, 0x6b, 0x35,
};
...
TEST_F(ExecutionTest, crc_calc) {
    ByteVector data;
    std::uniform_int_distribution<> byte_distribution{0, 0xff};
    std::mt19937 mt{0xDEADBEEF};

    std::generate_n(std::back_inserter(data), 0x4000,
                    [&]() { return static_cast<uint8_t>(byte_distribution(mt)); });

    auto crc = std::vector<uint8_t>{crc8(data)};
